...
class FileCryptorTest(unittest.TestCase):
    ...
    def test_decrypt_file_verified_should_raise_value_error_if_password_incorrect(self):
        ...
        self.cryptor.encrypt_file(self.filepath['test'], verify=True)

        with self.assertRaises(ValueError):
            # test
            self.cryptor.key = self.cryptor.hash_key("bad key!")
            self.cryptor.decrypt_file(self.filepath['encryption-test-result'], self.filepath['decryption-test-result'], verify=True)

    def test_decrypt_file_should_not_raise_value_error_if_password_incorrect(self):
        # setup
        shutil.copyfile(self.filepath['encrypted'], self.filepath['test'])
        self.cryptor.encrypt_file(self.filepath['test'], verify=False)

        # test doesn't raise Value Error
        self.cryptor.key = self.cryptor.hash_key("bad key!")
        self.cryptor.decrypt_file(self.filepath['encryption-test-result'], self.filepath['decryption-test-result'], verify=False)
    ...
    def test_verify_file_should_return_true_if_key_and_file_are_valid(self):
        # setup - create test file
        self.cryptor.encrypt_file(self.filepath['unencrypted'], self.filepath['test'], verify=True)

        # test
        result = self.cryptor.verify_file(self.filepath['test'])
...
class CryptorTest(unittest.TestCase):
    ...
    def test_encrypt_verified_and_decrypt_verified_should_be_reversible(self):
        ...
        # test
        encrypted_value = self.cryptor.encrypt(value, verify=True)
        decrypted_value = self.cryptor.decrypt(encrypted_value, verify=True)
    ...
    def test_decrypt_should_raise_value_error_if_bad_key(self):
        # setup
        encrypted_value = self.cryptor.encrypt("You are my sunshine...", verify=True)

        # test
        with self.assertRaises(ValueError):
            self.cryptor.key = self.cryptor.hash_key("bad key!")
            self.cryptor.decrypt(encrypted_value, verify=True)

    def test_decrypt_should_not_raise_value_error_if_bad_key(self):
        # setup
    ...
    def test_verify_should_return_digest_size_if_key_and_msg_are_valid(self):
        ...
        # test
        result = self.cryptor.verify(encrypted_value)
...
class Sha512DigestModFileCryptorTest(FileCryptorTest):
    def initialize_cryptor(self):
        self.cryptor = FileCryptor(key=TEST_KEY, digestmod=hashlib.sha512)

class DESCipherTest(CryptorTest):
    def setUp(self):
        self.cryptor = Cryptor(key=TEST_KEY, cipher_cls=DES)

class DESFileCipherTest(FileCryptorTest):
    def initialize_cryptor(self):
        self.cryptor = FileCryptor(key=TEST_KEY, cipher_cls=DES)

class BlowfishCipherTest(CryptorTest):
    def setUp(self):
        self.cryptor = Cryptor(key=TEST_KEY, cipher_cls=Blowfish)

class BlowfishFileCipherTest(FileCryptorTest):
    def initialize_cryptor(self):
        self.cryptor = FileCryptor(key=TEST_KEY, cipher_cls=Blowfish)
